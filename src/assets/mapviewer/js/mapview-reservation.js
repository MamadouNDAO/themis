...
let Salles = JSON.parse(localStorage.getItem('ListRooms'))

console.log(Salles)

function setColorsalles(goto = false) {

  let tableauCles = Object.keys(essential.content.places);
  if (localStorage.getItem('roomid')) {
    let __id2 = localStorage.getItem('roomid');
    const yRoom = tableauCles.find((element) => {
      let parties = element.includes("-") ? element.split("-") : [element];
      let partie1 = parties[0];
      let partie2 = parties[1] || null;
      return __id2 === partie1 || __id2 === partie2;
    });
  ...
  Salles.map(el => {
    //console.log(el.zone)
    let __id3 = el;
    const yRoomX = tableauCles.find((element) => {
      let parties = element.includes("-") ? element.split("-") : [element];
      let partie1 = parties[0];
      let partie2 = parties[1] || null;
      return __id3.zone.mapwizeId === partie1 || __id3.zone.mapwizeId === partie2;
    });

    if(yRoomX){
      if(el.reservationStatus.state === 'available'){
  ...
  /*let selectedSalle = mysalles[3];
  essential.content.setPlaceColor({
    id: mysalles[0].id,
    color: mysalles[0].color
  });
  essential.content.setPlaceColor({
    id: mysalles[1].id,
    color: mysalles[1].color
  });
  essential.content.setPlaceColor({
    id: mysalles[2].id,
    color: mysalles[2].color
  });*/
  /*if (goto) {
   // essential.venue.goToFloor({
      id: selectedSalle.floor
    });
    if (localStorage.getItem('roomid')) {
      let __id = localStorage.getItem('roomid');
      essential.venue.goToPlace({
        id: __id
      });
      if (!__id.startsWith("Table") && !__id.startsWith("Desk")) {
        essential.content.setPlaceIcon({
          id: __id,
          url: './marker_position.png'
        })
      }
    } else {
      let __id = selectedSalle.id;
      essential.venue.goToPlace({
        id: selectedSalle.id
      });
      if (!__id.startsWith("Table") && !__id.startsWith("Desk")) {
...
function setActivePlace(place) {
  ...
  // console.log(place)
  // console.log(essential.content.getPlace({id: 'Agora'}))
  // console.log(essential.content)
  ...
  requestAnimationFrame(() => {
    ...
    btnReservation.addEventListener("click", () =>  {
      pop.style.display = 'none'
      let parties = p.id.includes("-") ? p.id.split("-") : [p.id];
      let partie1 = parties[0];
      let partie2 = parties[1] || null;

      const xRoom = Salles.find((element) => {
        return element.zone.mapwizeId === partie1 || element.zone.mapwizeId === partie2;
      });
      localStorage.setItem('IdSalleCarto', xRoom.id)
      localStorage.setItem('RoomToReserve', JSON.stringify(xRoom))
    });
    ...
    btnConstat.addEventListener("click", () =>  {
      ...
      let partie1 = parties[0];
      let partie2 = parties[1] || null;
      const yRoom = Salles.find((element) => {
        return element.zone.mapwizeId === partie1 || element.zone.mapwizeId === partie2;
      });
      //console.log(yRoom.id)
      localStorage.setItem('IdSalleCartoConstat', yRoom.id)
    });



    //essential.content.setPlaceColor({id: p.id, color: primary})
    /*essential.content.resetPlaceColor({
      id: p.id
    })*/
    // essential.content.setHighlightedPlace({id: p.id, color: primary})
    //essential.content.resetHighlightedPlace()

    /*if (!p.id.startsWith("Table") && !p.id.startsWith("Desk")) {
      essential.content.setPlaceIcon({
        id: p.id,
        url: './marker_position.png'
      })
...
function setupSelectors() {
  ...
  if (venueLayout.buildings.length > 0) {
    ...
    currentFloorElement.innerText = 'REZ-DE-CHAUSSÉE';
    essential.venue.goToFloor({
      id: defaultFloor.id //'B1-UL37'
    });
    ...
    var insertFloors = function (buildingID) {
      if (venueLayout.buildingByID[buildingID].floors.length > 1) {
        ...
        //console.log('floor: '+JSON.stringify(bfloors))
        //let sortedFloors = _.reverse(_.sortBy([bfloors[6]], 'levelIndex'));
        ...
        //floorNames = floorNames.filter(x => x.id === 'B1-UL37');
        _.each(floorNames, ({
                              id,
                              name
                            }) => addListItem({
